...
class DinamicWindowAproach:
    def __init__(self):
        ...
        '''
        Parameters of the obstacles' moving error matrix
        '''
        wx_std_dev=0.3      # The standard deviation x[m] of the input error
        wy_std_dev=0.3      # The standard deviation y[m] of the input error
        wyaw_std_dev=0.1    # The standard deviation yaw[rad] of the input error
    ...
    def __move_obstacles(self):        
        # Move obstacles
        for i in range(len(self.__obstacle_list)):
            # input error
            w = np.random.multivariate_normal([0.0, 0.0, 0.0], self.__Q, 1)
            self.__obstacle_list[i, 0] += w[0, 0]
            self.__obstacle_list[i, 1] += w[0, 1]
    ...
    def __is_collision_with_obstacles(self, state):
        ...
        for i in self.__detected_objects:
            ...
            if distance < i[2] + self.__robot_radius:
                return True
