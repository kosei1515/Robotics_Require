...
class AStar():
    def __init__(self, start_x, start_y, goal_x, goal_y):
        ...
        self.__open_list[self.__calc_grid_index(self.__start_node)] = self.__start_node  
    ...
    def __calcurate_cost(self, motion_type, selected_node):
        '''
        Calcurate cost f(n)
        Input:
        Output: f_cost    The cost of the selected node
        '''
        f_cost = motion_type[2] + selected_node.cost
        
        return f_cost
    
    
    def __calcurate_heauristic_cost(self, selected_node):
        '''
        Calcurate heauristic cost h(n)
        Input:
        Output: h_cost    The heauristic cost of the selected node
        '''
        h_cost = self.__weight_h * np.sqrt((selected_node.x-self.__goal_node.x)**2 + (selected_node.y - self.__goal_node.y)**2)
        
        return h_cost
    
    def __is_goal(self, select_node_index):
        '''
        Check whether the selected node is the goal node or not. 
        Input: 
            select_node_index: the node index selected from the open list
        '''
        if select_node_index == self.__calc_grid_index(self.__goal_node):
            return True
        else:
            return False
    ...
    def __callback_parents(self, selected_node):
        ...
        # Get the parent node's index of the goal node
        index = selected_node.parent_index
        start_index = self.__calc_grid_index(self.__start_node)
        ...
        # Add the last parent node (start node)
        node = self.__close_list[index]   
        self.__node_list.append(node)
    ...
    def __AStar_plannig(self):        
        ...
        if self.__is_goal(min_cost_node_index):
            # Call back all parents until the start node
        ...
        for i, _ in enumerate(self.__motion):
            ...
            if not searched_node_index in self.__open_list:
                h_cost = self.__calcurate_heauristic_cost(searched_node)
                searched_node.cost += h_cost
                self.__open_list[searched_node_index] = searched_node
            else:
                h_cost = self.__calcurate_heauristic_cost(searched_node)
                if searched_node.cost + h_cost < self.__open_list[searched_node_index].cost:
                    self.__open_list[searched_node_index].cost = searched_node.cost +h_cost
                    self.__open_list[searched_node_index].parent_index = min_cost_node_index
    ...
    def main(self):
        ...
        while 1: 
            # 1 step planning
            selected_node, node_list = self.__AStar_plannig()
            ...
            if self.parent_callback_flag:
                #If parent callback is called, terminate planning
                break
...
if __name__ == '__main__':
    ...
    goal_x = 1.0
    goal_y = 48.0
    astar_planning = AStar(start_x, start_y, goal_x, goal_y)
